...
Вот несколько конкретных примеров, которые mimo должен поймать:

1. **Строка 6 и строка 14** — ПАВЕЛ ВАСИЛЬЕВ (Михайлович, мск, ИП Козлова) и ТИМУР ВАСИЛЬЕВ (Владимирович, г.Москва). Разные люди с одной фамилией, но оба написаны КАПСОМ — mimo должен нормализовать регистр, не склеивая их. А вот Васильева Екатерина (строка 2506) — третий человек с той же фамилией.
...
3. **Строка 10 и строка 2516** — татьяна комарова (г. Самара, без email) и Наталья Комарова (г. Новосибирск, с email). Разные имена, одна фамилия — это НЕ дубль. mimo не должен их склеивать.

## Время

На mimo Pro/Max: ~1–2 минуты (анализ) + ~1–2 минуты (нормализация).
...
Показать, что MiMo-Code справляется с грязными данными, которые в Excel пришлось бы чистить вручную часами. Описываешь проблему словами — mimo находит и исправляет.

## Подготовка

1. Открыть терминал в папке `crm-cleanup/`
2. Запустить MiMo-Code
...
mimo прочитает файл и выдаст отчёт о проблемах. Покажите это первым — студенты увидят, что mimo сам находит паттерны.
...
После первого промпта (анализ) mimo выдаст что-то вроде:
...
1. mimo нашёл проблемы сам. Не пришлось говорить «там Москва написана по-разному» — он сам увидел 8 вариантов.

2. Латинская «o» в «Мoсква». Частая проблема в данных — символы выглядят одинаково, но имеют разные коды. mimo это ловит.

3. Дубли — не точные совпадения. Один человек может быть записан с отчеством и без, с разными телефонами. mimo использует нечёткое сравнение.

4. 5200 строк — не предел. mimo спокойно работает с файлами на десятки тысяч строк. Ограничение — память контекста, но для табличных данных порог высокий.

5. Скрипт можно запускать повторно. mimo написал Python-скрипт — его можно использовать для новых выгрузок.
...
mimo обработал только первые 100 строк:
```
...
| Ситуация | Что делать |
|----------|------------|
| mimo обрабатывает только часть файла | Убедиться, что он прочитал весь CSV. Сказать: «Обработай все 5200 строк, не только первые» |
