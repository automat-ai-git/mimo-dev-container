...
Если в предыдущих демо использовался проектный MEMORY.md из `claude-md-intro/project-example.md`, оставьте его — в нём реквизиты компании «Вектор», формат дат, правила оформления документов. Если нет — демо всё равно сработает, но mimo придётся давать реквизиты вручную.
...
- Даты начала и окончания — mimo должен учесть их в договоре
...
Пока mimo работает, объясняем:
- mimo читает данные клиента из CSV
- Подтягивает описание услуги и цену из service-list.md
- Если есть MEMORY.md — берёт оттуда реквизиты и формат
...
Обращаем внимание: mimo обрабатывает всех клиентов за один запрос. В ручном режиме это 10 раз открыть шаблон, заполнить данные, проверить, сохранить.
...
1. mimo берёт данные из двух источников (таблица + прайс) и сопоставляет их. Копировать данные вручную не нужно
...
О: Положите шаблон в папку и укажите mimo: «Используй шаблон из contract-template.docx». mimo заполнит плейсхолдеры данными клиента.

**В: Можно ли генерировать в Word/Excel?**
О: Да. mimo может создавать .docx и .xlsx файлы. Укажите формат в промпте.

**В: Как быть с ЭДО?**
О: mimo генерирует документы как файлы. Загрузка в ЭДО (Диадок, СБИС) — отдельный шаг. На занятии 3 покажем, как подключать внешние сервисы через MCP.

**В: А если клиент из Казахстана — другая валюта?**
О: Укажите в промпте или добавьте колонку «валюта» в CSV. mimo адаптирует документы.

## Если что-то пошло не так

| Ситуация | Что делать |
|----------|------------|
| mimo перепутал клиентов | Проверьте что в CSV нет дублей и что mimo прочитал нужную строку. Можно уточнить: «Клиент #2, ИНН 7702345678» |
| Неправильная сумма НДС | mimo иногда считает НДС неверно (20% от суммы вместо выделения из суммы). Уточните: «Сумма 180 000 руб — это уже с НДС. Выдели НДС: 180 000 / 1.2 * 0.2 = 30 000 руб» |
| Реквизиты клиента неполные | В CSV есть только ИНН. Остальные реквизиты mimo выдумает. Предупредите аудиторию: в реальной работе реквизиты берут из договора или запрашивают у клиента |
| mimo создал все документы в один файл | Попросите разделить: «Каждый документ в отдельный файл» |
