...
```
mimo config set --global enableAgentTeams true
```

Перейдите в папку с демо и запустите MiMo-Code:
...
**Агенты работают медленно (дольше 20 минут).** Проверьте, не застряли ли они в цикле согласования. Если два агента бесконечно обсуждают формат данных, вмешайтесь: «Используйте ISO 8601 для дат и имена мастеров как строки. Продолжайте.» Если GLM 5 работает слишком медленно, покажите разницу на mimo Opus 4.6 с тем же промптом.
