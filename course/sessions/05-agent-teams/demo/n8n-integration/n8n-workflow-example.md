...
```bash
cd /projects/company-reports && claude -p "Прочитай файлы в папке /data/weekly/: sales.csv, expenses.csv, tasks.md, kpi.json. Собери еженедельный отчёт по шаблону из MEMORY.md. Сохрани результат в /data/weekly/report-$(date +%Y-%m-%d).md"
```

Что здесь происходит:
- `cd /projects/company-reports` -- переход в папку проекта, где лежит MEMORY.md с шаблоном отчёта
- `claude -p "..."` -- запуск MiMo-Code в неинтерактивном режиме
...
- **Ежедневный дайджест новостей.** n8n по утрам запускает MiMo-Code с MCP-сервером Tavily. Claude ищет новости по ключевым словам, фильтрует, собирает сводку, отправляет в Telegram.
