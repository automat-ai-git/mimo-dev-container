...
Каждый понедельник в 9:00 n8n запускает MiMo-Code, который собирает еженедельный отчёт из нескольких файлов и отправляет результат в Telegram.
...
```bash
cd /projects/company-reports && mimo -p "Прочитай файлы в папке /data/weekly/: sales.csv, expenses.csv, tasks.md, kpi.json. Собери еженедельный отчёт по шаблону из MEMORY.md. Сохрани результат в /data/weekly/report-$(date +%Y-%m-%d).md"
```

Что здесь происходит:
- `cd /projects/company-reports` -- переход в папку проекта, где лежит MEMORY.md с шаблоном отчёта
- `mimo -p "..."` -- запуск MiMo-Code в неинтерактивном режиме
- mimo читает 4 файла, собирает отчёт по шаблону и сохраняет его с датой в имени
...
- **Ежедневный дайджест новостей.** n8n по утрам запускает MiMo-Code с MCP-сервером Tavily. mimo ищет новости по ключевым словам, фильтрует, собирает сводку, отправляет в Telegram.
- **Проверка данных.** Каждый вечер mimo проверяет выгрузку из CRM на дубли и ошибки, формирует список для исправления.
- **Напоминание о дедлайнах.** n8n каждое утро запускает mimo, который читает файл с задачами и отправляет персональные напоминания тем, у кого дедлайн сегодня или завтра.
...
5. В Execute Command указать команду `mimo -p "ваш промпт"`
