...
Hooks — автоматические действия, которые MiMo-Code выполняет при определённых событиях. Настраиваются в `.mimocode/settings.json` (проектный уровень) или через `/hooks` в интерактивном режиме.
...
- **Notification** — при уведомлении от MiMo-Code (можно отправить системное уведомление)

### Что показываем

Откройте `.mimocode/settings.json` и разберите три примера:
...
**Slash-команда не находится.** Файл должен лежать строго в `.mimocode/commands/` в корне проекта. Если MiMo-Code запущен из другой директории, команда не найдётся.
